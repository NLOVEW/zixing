...
class AssistantController extends Controller{
    function index(){
        if (!session('?userID')) {
            ...
            $user = $assistant->query("select * from assistant WHERE as_name='$userName'");
            if ($userID == $user[0]['as_phone']) {
...
//         个人信息页面
    function personal(){
        $userID=session('userID');
        $assistant=M('assistant');
        $data=$assistant->query("select * from assistant WHERE as_phone='$userID'");
...
//        完成密码修改
    function completeChange(){
        $user=M('assistant');
        $id=session('userID');
        $data=$user->query("select * from assistant where as_phone='$id'");
        $pwd=$data[0]['as_password'];
        $oldPwd=$_POST['oldPwd'];
        $newPwd=$_POST['newPwd'];
        if($pwd==$oldPwd){
            $user->execute("update assistant set as_password='$newPwd' WHERE as_phone='$id'");
